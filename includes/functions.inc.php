...
function emptyInputSignUp($email, $username, $password, $country) {
...
function createUser($conn, $username, $password, $email, $country) {
    $sql = "INSERT INTO users (userName, userPassword, userEmail, userCountry) VALUES (?, ?, ?, ?);";
    ...
    mysqli_stmt_bind_param($stmt, "ssss", $username, $hasedPwd, $email, $country);

    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
    header("location: ../signup_page.php?error=none");

    $sql = "INSERT INTO country (userCountry) VALUES (?);";
    $stmt = mysqli_stmt_init($conn);
    if(!mysqli_stmt_prepare($stmt, $sql)) {
        header("location: ../signup_page.php?error=stmtFailedcreateUser");
        exit();
